...
2. Какому устройству соответствует значение Ethernet-адреса?
   ...
   - Принадлежит сетевому адаптеру моего компьютера
3. Каково значение исходного Ethernet-адреса? Какое устройство имеет такой Ethernet-адрес?
   - a4:a9:30:7b:eb:b5
   - Адрес маршрутизатора, видимо моего роутера
...
1. Каковы шестнадцатеричные значения исходного и конечного адресов в Ethernet-кадре,
   ...
   - bc:d0:74:ac:6f:f8 и ff:ff:ff:ff:ff:ff
...
#### Решение
1. Достаточно найти x. С вероятностью $q = Np(1-p)^{N-1}$ канал будет успешно захвачет. В схеме бернулли математическое ожидание неудач до первого успеха $=(1 - q)/q$.  Тогда эффективность = $$\frac{k}{k + (1-q)/q} = \frac{k}{k + \frac{1 - Np(1-p)^{N-1}}{Np(1-p)^{N-1}}}.$$
2. Возьмем производную по N:
$$\left(\frac{k}{k + \frac{1 - Np(1-p)^{N-1}}{Np(1-p)^{N-1}}}\right)' = \frac{-k N (1 - x)^N (-1 + N x)}{\text{что-то}}.$$
Снова максимум в точке $p = \frac{1}{N}$.
3. При $p = \frac{1}{N}$, $q = (1-\frac{1}{N})^{N-1}\rightarrow \frac{1}{e}$. Эффективность 
$$\frac{k}{k + (1-q)/q} = \frac{k}{k + \frac{1 - \frac{1}{e}}{\frac{1}{e}}} = \frac{k}{k + e -  1}.$$
4. Это очевидно. Так как мы делим на все большее и большее число.
$$1 - \frac{k}{k + (1 - q)/q} =  \frac{(1 - q)/q}{k + (1 - q)/q} \rightarrow 0$$
$$\frac{k}{k + (1 - q)/q} \rightarrow 1.$$
...
#### Решение
1. Задержка пакетирований
$$\frac{L+5}{\frac{128 \cdot 1000}{8}} \cdot 1000 = \frac{L + 5}{16}\text{ мс }.$$
2. При $L = 1500$ байт задержка $94.06$ мс, а при $L = 50$ байт задержка $3.44$ мс.
3. Нужно к предыдущим задержкам добавить $$\frac{L+5}{\frac{622 \cdot 1000000}{8}} \cdot 1000 = \frac{L+5}{77750}.$$
Соответствующие задержки 0.0194 и 0.0007.
4. Задержки пакетирования и задержки продвижения пакетов с промежуточным хранением сильно меньше. Есть возможно и другие преимущества: вероятность ошибки меньше, служебных данных меньше.
