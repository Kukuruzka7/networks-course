...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Time to live (каждый 3 раз), Identification, Header Checksum, Destination Address
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняются: Total Length, Version, Header Length, Flags, Differentiated Services Field, Fragment Offset, Source Address, Protocol
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Делается +1
      - Кажется в общем случае это не правда, и все зависит от реализации в узле.
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?
    Если мы говорим про icmp ответ:
   - Identification: Oxb1f5 (28590) — идентификация уникальности фрагмента IP-дейтаграммы, 2 байта.
   - Time to Live: 64 — время жизни пакета, 1 байт.
   ![](./img/first-response.png)
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Всего таких ответов от первого маршрутизатора (172.20.10.1) 3 штуки: Identification меняется, а TTL остается = 64.
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   - разные бывают и 253, и 62
   ![](./img/next-response.png)
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Да было, 3 фрагмента
      ![](./img/large-request.png)
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - При фрагментации меняются в дейтаграмме IPv4: флаги и смещение фрагмента, Total Length в нашем случае не равный, Header Checksum
