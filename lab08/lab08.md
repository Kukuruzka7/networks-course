...
#### Решение
Будем считать, что этот промежуток не в медленном старте. Средний размер окна $= \frac{3W}{4}$. 
Тогда средняя пропускная способность $\frac{3W}{4}$ в RTT. Если это не слишком строгое доказательство 
можно взять интеграл, все равно получится $\frac{3W}{4}$.
...
#### Решение
Время на установления TCP-соединения $2𝑅𝑇𝑇$. Нужно добавить ко всем ответам.
- Пункт 3. Первому пакету потребуется $\dfrac{𝑆}{𝑅} + 𝑅𝑇𝑇$. Дальше окно будет размера хотя бы 2. Следующие пакеты будут 
отправляться последовательно один за другим, потому что $𝑅𝑇𝑇$ маленькое и следующий пакет будет уже на очереди для отправки. 
И в конце надо подождать подтверждение $𝑅𝑇𝑇$. Итого:$15 \cdot \dfrac{𝑆}{𝑅} + 2𝑅𝑇𝑇$ 
- Пункт 2. Первый пакет все еще требует $\dfrac{𝑆}{𝑅} + 𝑅𝑇𝑇$. Из неравенства следует, что 3 пакет отправится, а потом придет 
подтверждение за 2 пакет и окно увеличится до 4, на этом этапе потратиться $\dfrac{𝑆}{𝑅} + 𝑅𝑇𝑇$. 
Далее снова из неравенства следует, что 7 пакет отправится, а потом придет 
подтверждение за 4 пакет и окно увеличится до 8, на этом этапе потратиться $\dfrac{𝑆}{𝑅} + 𝑅𝑇𝑇$. 
Дальше просто отправка 8 пакетов + ожидание подтверждения за последний, $8\cdot\dfrac{𝑆}{𝑅} + 𝑅𝑇𝑇$.
Итого: $11\cdot\dfrac{𝑆}{𝑅} + 4𝑅𝑇𝑇$.
- Пункт 1. Первый пакет все еще требует $\dfrac{𝑆}{𝑅} + 𝑅𝑇𝑇$. Из неравенства следует, что 3 пакет отправится, а потом придет 
подтверждение за 2 пакет и окно увеличится до 4, на этом этапе потратиться $\dfrac{𝑆}{𝑅} + 𝑅𝑇𝑇$. Дальше из неравенства 
$\dfrac{3S}{R} > RTT$, подтверждение за 4 пакет придет в момент когда мы все еще отправляем пакеты и окно увеличится до 8. 
Мы дальше мы будем непрерывно передавать пакеты. В конце еще надо подождать подтверждение. Итого $(2 + 12)\cdot\dfrac{𝑆}{𝑅} + 3𝑅𝑇𝑇$
...
#### Решение
Пусть $n=\log_{a+1} 2$. Тогда всего было отправлено пакетов примерно: 
$$N = \frac{w}{2} \cdot(1 + (1 + a) + \ldots + (1 + a)^n) \approx \frac{w}{2} \cdot(\frac{1 + a}{a} + 1)$$
Частота потери пакета $L = \frac{1}{N} = \frac{2wa}{2a+1}$.
...
#### Решение
Не очень понятно, что тут доказывать. Клиент устанавливает соединение с внешним сервером, на это у него уходит 
$1 \cdot RTT_{FE}$. Затем этот внешний сервер по уже открытому TCP соединению за $RTT_{BE}$ получает ответ от дата-центра, 
который в свою очередь потратил время обработки. $3 \cdot RTT_{FE}$ на получение клиентом данных от внешнего вервера.
