...
#### Решение
$d_{распр} = 0.005$, $d_{преобраз.А} = d_{преобраз.Б} = \frac{56 \cdot 8}{128000} = 0.0035$, $d_{передачи} = \frac{56 \cdot 8}{1000000} = 0.000448$. 

Итого: $d_{распр} + d_{преобраз.А} + d_{преобраз.Б} + d_{передачи} = 0.012448$.

**Ответ:** 0.012448 секунд.
...
#### Решение
Время передачи одного пакет $=\frac{1}{100} = 0.01$
Из условий есть $N = 10 + 1 = 11$, $d=0.01 + 0.01$, тогда $a = 550$ пакетов в секуду в среднем.

**Ответ:** 550 пакетов в секуду.
...
#### Решение
1. Единственное отличие во времени, которое происходит у этих двух пакетов: второй пакет 
    ждет пока первый пройдет по первой линии. **Ответ:** $\frac{L}{R_S}$.
2. В данном случае второй пакет попадает в ожидание в середине маршрута. Так как он отставал ровно на $\frac{L}{R_S}$, а первый пакет в маршрутизаторе отправляется $\frac{L}{R_C}$. Ну и отсюда сразу находится $T = \frac{L}{R_C} - \frac{L}{R_S}$. **Ответ:** $T = \frac{L}{R_C} - \frac{L}{R_S}$.
...
#### Решение
1. $\Delta = \frac{L}{R1} + \frac{L}{R2} = \frac{850000}{15 \cdot 10^6} + \frac{850000}{10^8} = 0.06516666666666666$
2. Среднее время ответа $= \frac{0.0651666}{1 - 16\cdot 0.065166} + 3$ и как-то внезапно первая часть этой суммы $<0$. Так не должно быть!
3. Если мы попали в кэш, то время ожидания 0.0085, если не попали, то пересчитав по вормуле с $B = 16 * 0.4$ получаем 0.11179 + 3.
Таким образом среднее $0.4 \cdot 0.0085 + 3.11179 \cdot 0.6 = 1.870474$.
