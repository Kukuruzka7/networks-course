...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   в окне деталей заголовка пакета.
    - 192.168.1.185 и 49994
      ![](img/1.png)
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   TCP-сегментов этого соединения?
    - 128.119.245.12
    - общение между 49994 портом (49993 порт видимо взаимосвязан с нажатием на кнопку) и 80 портом сервера
      ![](img/2.png)
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   Как определяется, что это именно SYNACK-сегмент?
    ...
    - Номер следующего байта для передачи
    - битики Syn и Acknowledgment выставлены
      ![](img/4.png)
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент
   ...
   получено каждое подтверждение, чему равно значение RTT для каждого из 6 сегментов?
    - 1, 640, 2100, 3560, 5020, 6480
   
   | Номера сегментов | Время отправки | Получен ACK пакет                 | RTT      |
   |------------------|----------------|-----------------------------------|----------|
   | 1                | 8.943549       | 9.070233                          | 0.126684 |
   | 640              | 8.943606       | 9.070234 (формально его не было)  | 0.126628 |
   | 2100             | 8.943611       | 9.070234 (формально его не было)  | 0.126623 |
   | 3560             | 8.943615       | 9.070234  (формально его не было) | 0.126619 |
   | 5020             | 8.943619       | 9.070234  (формально его не было) | 0.126615 |
   | 6480             | 8.943622       | 9.070234  (формально его не было) | 0.126612 |
   ![](img/6.png)
...
По формуле с лекции мы хотим:

$$\frac{WinSize\cdot\frac{L}{R}}{\frac{L}{R} + RTT}\geq 0.98$$
$$ WinSize\geq 0.98\cdot\frac{L + R\cdot RTT}{L}$$
$$WinSize\geq 2451.$$
